...
test("loads and displays artist dates", async () => {
  const artist = {
    id: 9,
    name: "Paul Cezanne",
    dates: "1839-1906",
  ...
  const { getByTestId } = render(<Artist artist={artist} />);
  expect(getByTestId("artist-dates")).toHaveTextContent("1839-1906");
});

test("loads and displays artist bio", async () => {
  const artist = {
    id: 9,
    name: "Paul Cezanne",
    dates: "1839-1906",
  ...
  const { getByTestId } = render(<Artist artist={artist} />);
  expect(getByTestId("artist-bio")).toHaveTextContent(
